...
if (!function_exists('formatDateForView')) {
	 ...
	 * @return string
	 */
	function formatDateForView($date): string
	{
		if (!$date) {
			return '';
		}

		return $date->isoFormat('D') . ' ' . formatMonthForView($date);
	}
}

if (!function_exists('formatMonthForView')) {
	/**
	 * Format date month, short if large name is larger than 5 symbols
	 *
	 * @param Carbon $date
	 * @return string
	 */
	function formatMonthForView(Carbon $date): string
	{
		...
		if (mb_strlen($month) > config('view.month_chars_limit')) {
...
if (!function_exists('number2word')) {
	/**
	 * Function for determining word ending by numeric
	 *
	 * @param int $num
	 * @param array $words
	 * @return string
	 */
	function number2word(int $num, array $words) :string
	{
		$num = $num % 100;
		if ($num > 19) {
			$num = $num % 10;
...
if (!function_exists('declensionOfWords')) {
    /**
     * Declension of words
     *
     * @param int $num
     ...
     * @return string
     */
    function declensionOfWords(string $word): string
    {
        ...
        switch ($word) {
            ...
            case 'статья':
                $word = 'статью';
                break;
            case 'будущее мероприятие':
                $word = 'мероприятие';
                break;
            case 'прошедшее мероприятие':
                $word = 'мероприятие';
                break;
            case 'услуга':
                $word = 'услугу';
...
if (!function_exists('keySearchInMultidimensionalArray')) {
    /**
     * Find key by specific value
     *
     * @param array $array
     * @param $field
     * @param $value
     ...
     */
    function keySearchInMultidimensionalArray(array $array, $field, $value)
    {
...
if (!function_exists('str_limit_to_nearest_character')) {

    /**
     * Limit the number of characters in a string with trimming down to the nearest space.
     *
     * @param $string
     * @param int $limit
     * @param string $end
     ...
     * @return string
     */
    function str_limit_to_nearest_character($string, $limit = 100, $end = '...', $pattern = '/\s/')
    {
        if (mb_strwidth($string, 'UTF-8') <= $limit) {
            return $string;
        }

        $string = mb_strimwidth($string, 0, $limit, '', 'UTF-8');

        preg_match_all($pattern, $string, $matches, PREG_OFFSET_CAPTURE);

        if (is_array($matches) && key_exists(0, $matches)) {
            $position = end($matches[0])[1];
            $string = mb_strcut($string, 0, $position);
        }
...
if (!function_exists('truncateHTML')) {
     ...
     * @version 1.0.1
     */
    function truncateHTML(int $maxLength, string $html, array $options = [])
    {
...
if (!function_exists('build_table')) {
    function build_table($array, $enableHeader = true){
        // start table
        $html = '<table border="1">';
...
if (!function_exists('encode_basename')) {
