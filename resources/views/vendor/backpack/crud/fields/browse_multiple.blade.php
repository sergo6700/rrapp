...
switch (ModelUtils::getEntityNameFromModel($crud->entry)) {
    case "Document":
    $field['value'] = ModelUtils::getRelationFieldValues($crud->entry, 'files', 'path');
    break;
}
...
// keys indicating where there is an error
$keyErrors = [];
if (!empty($errors) && $errors->has($field['name'] . '.*')) {
    foreach ($errors->get($field['name'] . '.*') as $key => $arr) {
        $search = $field['name'].'.';
        $keyErrors[str_replace($search, '', $key)] = $arr[0];
    }
}
@endphp
...
<div @include('crud::inc.field_wrapper_attributes') >
    ...
    @if ($multiple)
        @foreach((array)$value as $index => $path)
            @if ($path)
                <div class="input-group input-group-sm @if (!empty($keyErrors) && key_exists($index, $keyErrors)) has-error @endif">
                    <input type="text" name="{{ $field['name'] }}[]" value="{{ $path }}" @include('crud::inc.field_attributes') readonly>
                ...
                @if (!empty($keyErrors) && key_exists($index, $keyErrors))
