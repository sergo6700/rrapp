...
<li filter-name="{{ $filter->name }}"
    ...
    <div class="dropdown-menu ajax-select" style="width: 250px;">
...
@push('crud_list_scripts')
	...
	<script src="{{ asset('vendor/backpack/select2/select2_locale_ru.js') }}" type="text/javascript"></script>
    <script>
        jQuery(document).ready(function($) {
            ...
            $('#filter_{{ $filter->name }}').select2({
				...
				if(input.length > 0) {
					let mainContainer = input.closest('.dropdown-menu');
					if (mainContainer.length > 0 || mainContainer.find('.select2-container')) {
						let select2Container = mainContainer.find('.select2-container')
						let newWidth = mainContainer.find('.select2-chosen').text().length * CHARACTER_LENGTH

						mainContainer.css( "width", newWidth)
						select2Container.css( "width", newWidth)
					}
				}
